...
You do not need to add a newly published policy to the values file by hand.
The next generation includes any policy directory that has a `Makefile` and is
not in `policies/excluded-from-publishing.txt`. Watch for the reverse case. If
a policy has no image yet under `ghcr.io/<owner>/policies/<policy-id>`, its
`dockerimage` source finds no matching tag. Then the weekly run fails. The
manifest target depends on every source. As a result, one missing image blocks
the whole update. Keep such a policy in `policies/excluded-from-publishing.txt`
